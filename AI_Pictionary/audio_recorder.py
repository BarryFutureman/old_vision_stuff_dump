...
class AudioRecorder:
    def __init__(self):
        ...
        self.record_process = threading.Thread(target=self.record_audio)
    ...
    def audio_sample_to_wav(self, sample_duration: int):
        ...
        directory_path = "wav/temp"
        filename = directory_path + f"/temp{int(time.time()-sample_duration)}to{int(time.time())}.wav"
